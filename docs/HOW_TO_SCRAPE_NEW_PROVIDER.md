...
This guide explains the foundational steps to scrape a _new_ streaming website and turn it into a working SkyStream JavaScript provider.
...
6.  **Action**:
    ...
    - If _no_ JSON calls appear and the data is in the HTML `Doc`, you will need to scrape the HTML using Regex.
...
3.  **Trace the Origin**:
    ...
    - **Iframe**: The site loads an external player (e.g., `vidcloud.net/embed/123`).
      - _Action_: You must write a `load()` function that fetches the movie page -> extracts the Iframe URL -> fetches the Iframe page -> extracts the `.m3u8` link.
...
function getHome(callback) {
  ...
  http_get(mainUrl, commonHeaders, (status, html) => {
    ...
    callback(JSON.stringify([{ title: "Home", Data: movies }]));
  });
}
```
...
- **Workaround**: Use Regex to extract the internal list if possible, or search for a different source. CloudStream JS engine _can_ technically run generic JS, but heavy deobfuscation scripts might timeout.
...
_Note: Check `js_engine.dart` to see if `http_post` is explicitly exposed. In the current iteration, we primarily use `http_get`._
...
- **Iframe**: A "website inside a website". Streaming sites use this to host the player on a different server (e.g., `vidcloud.net`) to avoid copyright takedowns. You scrape the _main_ site to find the _iframe link_, then scrape the _iframe_ to find the video.
