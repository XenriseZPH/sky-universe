...
**Logic**:
Sometimes the page you load contains the _real_ stream data in a hidden JSON or variable. You can extract that here and pass it via the `data` field to `loadStreams`.
...
[
  {
    ...
    "subtitles": [{ "label": "English", "url": "https://...vtt" }]
  }
]
```
...
3.  **Hidden JSONs**: Does the response contain a key like `l` or `source` that points to _another_ JSON file?
...
- **Constraint**: While QuickJS supports Promises, the specific _bridge_ implementation in CloudStream often relies on strictly sequenced **CS-Callbacks**.
