...
SkyStream runs on **Android, iOS, and Desktop (macOS/Windows/Linux)**. You can develop and test your plugins on _any_ of these platforms using the local testing workflow.
...
4.  **Register the Asset**:
    - **New Files**: If you just created `MyProvider.js`, you MUST stop and run `flutter run` again. Hot Restart will _not_ pick up new files in assets.
...
4.  **How it Works**:
    ...
    - _Note_: The icon is NOT loaded from the zip. You must host the `icon.png` separately and link it in your `plugins.json`.
...
```json
{
  ...
  "manifestVersion": 1,
  "pluginLists": ["https://example.com/repo/plugins.json"]
}
```
